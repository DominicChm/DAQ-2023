<script lang="ts">
    ...
    import { ArrowRight, CaretRight, MagnifyingGlass, PencilSimple } from "phosphor-svelte";
    ...
    <div class="flex justify-between prose w-auto max-w-none max-h-14">
        <div class="flex items-center gap-2  overflow-visible relative">
            <div class="bg-neutral flex items-center gap-2 px-3 m-0 py-2 rounded-br-2xl cursor-move select-none h-full z-10 revealer">
                <h3 class=" m-0">{testDataDef.chartName || "Unnamed Chart"}</h3>
                <CaretRight/>
            </div>
    ...
    <Portal>
        <div class="modal modal-open" transition:fade={{ duration: 100 }}>
            <div class="modal-box w-full max-w-full h-full max-h-full flex flex-col rounded-none">
                ...
                <h3 class="font-bold text-lg">{testDataDef.chartName}</h3>
